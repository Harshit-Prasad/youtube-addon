...
export default function AdminToggle() {
  ...
  return (
    <button
      ...
      disabled={loading}
      className={`block relative h-[2rem] w-[4rem] rounded-full ${toggle ? "bg-green-500" : "bg-red-500"
        } disabled:bg-slate-600 `}
    >
      <span
        className={`absolute h-6 w-6 rounded-full top-[4px] left-[4px] bg-white ${toggle && "translate-x-8"
          }  transition-all`}
      ></span>
    </button>
  );
